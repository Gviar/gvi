...
async def handle_ru_commands(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for cmd in ['дуэль ', 'брак ', 'свадьба ', 'развод ', 'расставание ', 'альянс ', 'союз ',
                ...
                'мафияпроверка ', 'мафияхил ', 'мафияудар ', 'мафияблок ']:
...
def build_quote_sticker_image(text: str, author: str) -> BytesIO | None:
    if not PIL_OK:
        return None
    canvas = Image.new("RGBA", (512, 512), (20, 20, 24, 255))
    ...
    if not lines:
        lines = ["..."]
    # авто-уменьшение шрифта, чтобы текст не был микроскопическим и влезал
    while len(lines) > 8 and hasattr(font, "size") and font.size > 26:
        font = _load_font(font.size - 2)
        lines = []
        cur = ""
        for w in words:
            test = (cur + " " + w).strip()
        ...
        if cur:
            lines.append(cur)
    y = 120
    draw.rounded_rectangle((24, 24, 488, 488), radius=30, fill=(26, 28, 38, 255), outline=(255, 255, 255, 36), width=2)
    draw.ellipse((44, 44, 112, 112), fill=(73, 142, 255, 255))
    draw.text((66, 60), "Q", font=_load_font(32), fill=(255, 255, 255, 255))
    for ln in lines[:10]:
        draw.text((46, y), ln, font=font, fill=(255, 255, 255, 255), stroke_width=1, stroke_fill=(0, 0, 0, 120))
        y += int(font.size * 1.22) if hasattr(font, "size") else 34
    draw.text((46, 446), f"— {author}", font=small, fill=(180, 220, 255, 255))
    bio = BytesIO()
    bio.name = "quote.webp"
...
async def handle_sticker_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(txt) > 380:
        txt = txt[:380] + "…"
    author = msg.from_user.first_name or (msg.from_user.username or "user")
    image = build_quote_sticker_image(txt, author)
    if image is None:
        await msg.reply_text("Для стикеров нужен Pillow. Добавь `Pillow` в requirements и перезапусти.")
        return
...
async def quote_sticker(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if src and src.text:
        ...
        author = src.from_user.first_name if src.from_user else "user"
    elif context.args:
    ...
    image = build_quote_sticker_image(txt, author)
    if image is None:
        await msg.reply_text("Не удалось создать стикер: проверь Pillow в requirements.")
        return
    await context.bot.send_sticker(chat_id=msg.chat_id, sticker=InputFile(image))
