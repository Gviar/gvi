...
async def handle_ru_commands(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if text in game_commands:
        ...
        no_args = [
            ...
            , 'ранг', 'мафия', 'мафиявойти', 'мафиястарт', 'мафиястатус', 'мафиястоп'
        ]
    ...
    for cmd in ['дуэль ', 'брак ', 'свадьба ', 'развод ', 'расставание ', 'альянс ', 'союз ',
                ...
                'обмен ', 'вклад ', 'снять ', 'передать ', 'мафияголос ']:
...
async def mafia_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    roles = {}
    for p in players:
        roles[p] = "мафия" if p in mafias else "мирный"
    g["roles"] = roles
    g["started"] = True
    g["alive"] = players[:]
    g["votes"] = {}
    ...
    for p in players:
        ...
        try:
            ...
            continue
    await reply_game(update, context, "Мафия началась. Голосование: мафияголос @user", 180)
...
async def mafia_stop(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not g.get("active"):
        ...
        return
    mafia_games[chat_id] = {"active": False, "started": False, "players": [], "roles": {}, "alive": [], "votes": {}, "round": 0, "host": ""}
