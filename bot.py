...
def _ceremony_id(chat_id: str, a: str, b: str) -> str:
    return f"{chat_id}:{min(a,b)}:{max(a,b)}"
...
async def brak(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    kb = InlineKeyboardMarkup(
        [[
            InlineKeyboardButton("✅ Принять брак", callback_data=f"marrycb:yes:{key}"),
            InlineKeyboardButton("❌ Отклонить", callback_data=f"marrycb:no:{key}"),
        ]]
    )
    await msg.reply_text(
...
async def start_marriage_ceremony(chat_id: int, a: str, b: str, context: ContextTypes.DEFAULT_TYPE) -> None:
    cid = _ceremony_id(str(chat_id), a, b)
    ceremony_state[cid] = {
        "chat_id": str(chat_id),
        "a": a,
        "b": b,
...
async def trade(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    kb = InlineKeyboardMarkup(
        [[
            InlineKeyboardButton("Принять обмен", callback_data=f"tradecb:ok:{tid}"),
            InlineKeyboardButton("Отклонить", callback_data=f"tradecb:no:{tid}"),
        ]]
    )
    await msg.reply_text(
