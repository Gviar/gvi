...
async def handle_ru_commands(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if text in ['помощь', 'help', 'команды', 'что ты умеешь']:
        ...
        return
    if text.startswith("мопс реши "):
        context.args = [msg.text.strip()[len("мопс реши "):]]
        await mops_ai_solve(update, context)
        return
    ...
    for cmd in ['дуэль ', 'брак ', 'свадьба ', 'развод ', 'расставание ', 'альянс ', 'союз ',
                ...
                'мафияпроверка ', 'мафияхил ', 'мафияудар ', 'мафияблок ', 'мопсреши ']:
...
def _fix_mojibake(text: str) -> str:
    ...
    replacements = {
        ...
        "повеСЂжен": "повержен",
        "РќагСЂады": "Награды",
        "РнвентаСЂь": "Инвентарь",
        "СеСЂёя": "Серия",
        "Рўакого": "Такого",
        "Рќужно": "Нужно",
        "Рќёчьё": "Ничьи",
        "ПоСЂаженёя": "Поражения",
    }
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "9) AI ОТ МОПСА:\n"
        "/mops_ai <задача> или текстом: мопс реши <задача>\n"
        "Требует OPENAI_API_KEY в Railway.\n\n"
        "10) СКРЫТЫЕ owner-команды:\n"
...
async def mops_ai_solve(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    prompt = " ".join(context.args).strip() if context.args else ""
    if not prompt:
        await msg.reply_text("Формат: мопс реши <задача> или /mops_ai <задача>")
        return

    api_key = (os.getenv("OPENAI_API_KEY") or "").strip()
    if not api_key:
        await msg.reply_text("AI-режим выключен: не задан OPENAI_API_KEY в переменных.")
        return

    payload = {
        "model": "gpt-4.1-mini",
        "input": f"Реши задачу кратко и понятно по-русски. Если это математика — покажи шаги.\nЗадача: {prompt}",
    ...
    try:
        ...
        if not answer:
            answer = "Не смог получить ответ, попробуй переформулировать задачу."
        await msg.reply_text("🐶 Мопс-ИИ:\n" + answer[:3500])
    except Exception as e:
        await msg.reply_text(f"Ошибка AI-режима: {e}")
...
def build_quote_sticker_image(text: str, author: str) -> BytesIO | None:
    ...
    # 1) Уменьшаем шрифт до читаемого минимума, чтобы текст влез по высоте.
    min_size = 28
    lines = wrap_lines(body_font, max_text_width)
    line_h = int((body_font.size if hasattr(body_font, "size") else 36) * 1.23)
    ...
    # 2) Если даже на минимуме текст длинный — аккуратно обрезаем по строкам + эллипсис.
    max_lines = max(4, max_body_height // max(1, line_h))
    ...
    line_h = int((body_font.size if hasattr(body_font, "size") else 36) * 1.23)
    bubble_h = max(360, 106 + line_h * len(lines) + 32)
    ...
    av_x1, av_y1, av_x2, av_y2 = 40, 44, 100, 104
    draw.ellipse((av_x1, av_y1, av_x2, av_y2), fill=(79, 142, 255, 255))
    initial = (author[:1] or "Q").upper()
    init_font = _load_font(30)
    draw.text((av_x1 + 19, av_y1 + 12), initial, font=init_font, fill=(255, 255, 255, 255))

    # Author + pseudo time-like right accent.
    name_color = (255, 132, 182, 255)
    draw.text((114, 52), author[:28], font=name_font, fill=name_color)
    draw.text((454, 56), "⋯", font=_load_font(30), fill=(208, 214, 255, 220))

    # Message text.
    y = y_start
    for ln in lines:
        draw.text((56, y), ln, font=body_font, fill=(250, 252, 255, 255))
        y += line_h
...
def build_quote_sticker_image_with_avatar(text: str, author: str, avatar_img) -> BytesIO | None:
    ...
    try:
        ...
        av = avatar_img.convert("RGBA").resize((44, 44))
        # Circle mask for avatar.
        mask = Image.new("L", (44, 44), 0)
        md = ImageDraw.Draw(mask)
        md.ellipse((0, 0, 44, 44), fill=255)
        base.paste(av, (48, 56), mask)
        out = BytesIO()
        out.name = "quote.webp"
...
async def hoshi_scene_job(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for chat_id, cfg in chats.items():
        ...
        if random.random() > 0.4:
            continue
        try:
            payload = random.choice(HOSHI_SCENES + DUO_TIPS)
...
def build_application(token: str) -> Application:
    ...
    app.add_handler(CommandHandler("mops_ai", mops_ai_solve))
