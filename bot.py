...
async def handle_ru_commands(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if text in {"дуэль принять", "дуэль согласен", "дуэль согласна"}:
        await accept(update, context)
        return
    if text in {"дуэль отклонить", "дуэль отказ"}:
        await decline(update, context)
        return
    if text in {"брак согласен", "брак согласна", "брак не согласен", "брак не согласна"}:
        await _marriage_decision_text(update, text.replace("брак ", "", 1))
        return
    ...
    if text in ceremony_words:
        pending_user = register_profile(msg.from_user)
        _, pending = find_pending_marriage_for_user(chat_id, pending_user)
        if pending:
            await marriage_ceremony_text(update, context)
            return
        if text in {"согласен", "согласна"}:
            await accept(update, context)
            return

    # Быстрые текстовые алиасы (без /) для популярных действий.
    text_alias_handlers = {
        "баланс": balance,
        "кошелек": balance,
        "профиль": profile,
        ...
        "союзы": soyuzy,
        "мои союзы": moisoyuz,
        "брак статус": marriage_status,
        "брак отмена": marriage_cancel,
        "обмен статус": trade_status,
        "обмен отмена": trade_cancel,
        "рп действия": relation_actions_list,
        "отношения топ": relation_top,
        "отношения дейли": relation_daily,
        "совместимость пары": pair_compat,
        "история пары": pair_story,
        "мопс статус": mops_status,
        "хоши статус": hoshi_status,
        "какаду шутка": kakadu_joke,
        "какаду челлендж": kakadu_challenge,
        "дуосцена": duo_scene,
        "дуообнять": duo_hug,
        "донат": donate_menu,
        "донат топ": donate_top,
    }
    if text in text_alias_handlers:
        await text_alias_handlers[text](update, context)
        return

    if text in {"шептун", "шепот", "шёпот", "whisper"}:
        await whisper_help(update, context)
        return
    if text.startswith("фармила-шепот ") or text.startswith("шепни "):
        cmd = text
        if text.startswith("фармила-шепот "):
            cmd = text.replace("фармила-шепот ", "", 1)
        elif text.startswith("шепни "):
            cmd = text.replace("шепни ", "", 1)
        context.args = cmd.split()
        await whisper_send(update, context)
        return

    if text.startswith("фармила-поиск") or text.startswith("поиск ") or text.startswith("гугл ") or text.startswith("найди "):
        await farmila_search(update, context)
        return
    if text.startswith("фармила-реши") or text == "реши":
        await farmila_reshi(update, context)
        return
    ...
    if text in ['донаттоп', 'топдонат']:
        ...
        return
    if text in ['отн действия', 'отношения действия', 'отндействия']:
        await relation_actions_list(update, context)
        return
    if text in ['совместимость пары', 'совместимость', 'пара совместимость']:
        await pair_compat(update, context)
        return
    if text in ['история пары', 'пара история']:
        await pair_story(update, context)
        return
    if text.startswith("отн пропуск "):
        context.args = text[len("отн пропуск "):].split()
        await relation_skip(update, context)
        return
    if text in ['бракстатус', 'свадьбастатус']:
        await marriage_status(update, context)
        return
    if text in ['бракотмена', 'свадьбаотмена']:
        await marriage_cancel(update, context)
        return
    ...
    game_commands = {
        ...
        'донат': donate_menu,
        'донаттоп': donate_top,
        'обменстатус': trade_status,
        'обменотмена': trade_cancel,
        'бракстатус': marriage_status,
        'бракотмена': marriage_cancel,
        'отндействия': relation_actions_list,
        'отнтоп': relation_top,
        'отндейли': relation_daily,
        'совместимость': pair_compat,
        'историяпары': pair_story,
    }

    if text in game_commands:
        func = game_commands[text]
        no_args = [
            'принять', 'отклонить', 'отказ', 'выстрел', 'стрель',
            ...
            , 'какадушутка', 'какадупати', 'какадунастроение', 'какадучеллендж', 'донат', 'донаттоп', 'обменстатус', 'обменотмена', 'бракстатус', 'бракотмена', 'отндействия', 'отнтоп', 'отндейли', 'совместимость', 'историяпары'
        ]
    ...
    for cmd in ['дуэль ', 'брак ', 'свадьба ', 'развод ', 'расставание ', 'альянс ', 'союз ',
                ...
                'мафияпроверка ', 'мафияхил ', 'мафияудар ', 'мафияблок ', 'ответ ', 'свидание ', 'подарок ', 'отчеты период ', 'какаду повтори ', 'отн ']:
        if text.startswith(cmd):
            ...
            elif cmd_stripped == 'отн':
                context.args = rest.split()
                await relation_do(update, context)
            return
...
PENDING_MARRIAGE_FILE = os.path.join(DATA_DIR, "pending_marriages.json")
...
DONATE_FILE = os.path.join(DATA_DIR, "donate_stats.json")
CEREMONY_FILE = os.path.join(DATA_DIR, "marriage_ceremony.json")
WHISPER_FILE = os.path.join(DATA_DIR, "whispers.json")

DONATE_AMOUNTS = [5, 10, 25, 50, 100, 250, 500, 1000]
RELATION_TITLES = [
    "Знакомые", "Дружеские отношения", "Теплые отношения", "Симпатия", "Флирт", "Ухаживания",
    "Отношения", "Серьезные отношения", "Брак", "Великая любовь", "Любовь всей жизни",
] + [f"Легенда любви #{i}" for i in range(1, 121)]

RELATION_ACTIONS = {
    "обнять": {"xp": 8, "cost": 0, "verb": "обнял(а)"},
    "поцеловать": {"xp": 12, "cost": 0, "verb": "поцеловал(а)"},
    "поговорить по душам": {"xp": 30, "cost": 15, "verb": "поговорил(а) по душам с"},
    "пригласить погулять": {"xp": 25, "cost": 12, "verb": "пригласил(а) погулять"},
    "сделать завтрак": {"xp": 40, "cost": 20, "verb": "сделал(а) завтрак для"},
    "устроить сюрприз": {"xp": 80, "cost": 40, "verb": "устроил(а) сюрприз для"},
    "сделать комплимент": {"xp": 18, "cost": 5, "verb": "сделал(а) комплимент"},
    "поделиться едой": {"xp": 20, "cost": 6, "verb": "поделился(ась) едой с"},
    "рассказать анекдот": {"xp": 16, "cost": 4, "verb": "рассказал(а) анекдот"},
    "поблагодарить": {"xp": 14, "cost": 0, "verb": "поблагодарил(а)"},
    "погладить": {"xp": 11, "cost": 0, "verb": "погладил(а)"},
    "покормить": {"xp": 17, "cost": 7, "verb": "покормил(а)"},
    "дать пять": {"xp": 7, "cost": 0, "verb": "дал(а) пять"},
    "прижать": {"xp": 10, "cost": 0, "verb": "прижал(а)"},
    "ущипнуть": {"xp": 5, "cost": 0, "verb": "ущипнул(а)"},
}
RELATION_ACTION_COOLDOWN_MIN = {
    "обнять": 2,
    "поцеловать": 4,
    "поговорить по душам": 20,
    "пригласить погулять": 25,
    "сделать завтрак": 35,
    "устроить сюрприз": 90,
    "сделать комплимент": 10,
    "поделиться едой": 12,
    "рассказать анекдот": 8,
    "поблагодарить": 6,
    "погладить": 5,
    "покормить": 14,
    "дать пять": 3,
...
CEREMONY_QUESTIONS = [
    "Готовы ли вы поддерживать друг друга в трудные моменты?",
    "Обещаете ли вы разговаривать и решать конфликты без обид?",
    "Готовы ли вы радоваться успехам партнера как своим?",
    "Согласны ли вы строить общее будущее вместе?",
    "Подтверждаете ли вы взаимное доверие и уважение?",
]
...
ceremony_state: dict[str, dict] = {}
whisper_state: dict[str, dict] = {}
mafia_games: dict[str, dict] = {}
sticker_sessions: dict[str, bool] = {}
marriage_cb_map: dict[str, str] = {}
trade_cb_map: dict[str, str] = {}
...
def clear_pending_marriages_for_users(chat_id: str, users: set[str]) -> None:
    ...
    if to_delete:
        save_json(PENDING_MARRIAGE_FILE, pending_marriages)


def _ceremony_result_table(state: dict) -> str:
    answers = state.get("answers", [])
    lines = ["Итог церемонии (да/нет):"]
    for i, row in enumerate(answers, start=1):
        q = row.get("q", "")
        a1 = "да" if row.get("a") == "yes" else ("нет" if row.get("a") == "no" else "—")
        b1 = "да" if row.get("b") == "yes" else ("нет" if row.get("b") == "no" else "—")
        lines.append(f"{i}. {q}")
        lines.append(f"   {display_user(state.get('a',''))}: {a1} | {display_user(state.get('b',''))}: {b1}")
    return "\n".join(lines)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "/trade_status, /trade_cancel — статус/отмена обмена\n"
        "/marriage_status, /marriage_cancel — статус/отмена заявки брака\n"
        ...
        "/farmila_search <вопрос> — умный поиск в интернете\n"
        "/farmila_reshi — решить задачу по фото (ответом на фото)\n"
        "/whisper_help — справка по шептуну\n"
        "/whisper_demo <текст> @user — тест шепота в чат\n"
        ...
        "8.1) ОТНОШЕНИЯ+: \n"
        "/relation_actions, /relation_do <действие> @user\n"
        "/relation_skip <действие> @user — пропуск кулдауна за монеты\n"
        "/relation_daily, /relation_top, /pair_compat, /pair_story\n"
        "Текстом: отн действия | отн <действие> @user | отн пропуск <действие> @user\n\n"
        "9) ХОШИ+ДУЭТ ДОП:\n"
        ...
        "/mxc /mxd /mxx /mxp /mxr /mxbackup /mxrestore (и текстовые фармила-...)\n"
        f"Хранилище данных: {DATA_DIR}\n"
    )
...
async def brak(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    target = resolve_user_key_in_chat(chat_id, mentioned[0]) if mentioned else None
    ...
    save_json(PENDING_MARRIAGE_FILE, pending_marriages)
    cbid = uuid4().hex[:10]
    marriage_cb_map[cbid] = key
    kb = InlineKeyboardMarkup(
        [[
            InlineKeyboardButton("✅ Принять брак", callback_data=f"marrycb:yes:{cbid}"),
            InlineKeyboardButton("❌ Отклонить", callback_data=f"marrycb:no:{cbid}"),
        ]]
    )
    await msg.reply_text(
...
async def marriage_ceremony_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await _marriage_decision_text(update, text)


async def _marriage_decision_text(update: Update, text: str) -> None:
    msg = update.message
    chat_id = str(msg.chat_id)
    ...
    if text in decline_words:
        pending_marriages.pop(key, None)
        save_json(PENDING_MARRIAGE_FILE, pending_marriages)
        await msg.reply_text(f"💔 Церемония отменена.\n{display_user(user)} не согласен(на) на брак.")
        return
    ...
    if request.get("a_ok") != "1" or request.get("b_ok") != "1":
        waiting_for = b if user == a else a
        await msg.reply_text(f"✅ {display_user(user)} подтвердил(а) брак. Ждем: {display_user(waiting_for)}")
        return
    ...
    pending_marriages.pop(key, None)
    save_json(PENDING_MARRIAGE_FILE, pending_marriages)
    await start_marriage_ceremony(msg.chat_id, a, b, context)


async def marriage_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    chat_id = str(msg.chat_id)
    user = register_profile(msg.from_user)
    key, req = find_pending_marriage_for_user(chat_id, user)
    if not req:
        await msg.reply_text("Активной свадебной заявки для тебя нет.")
        return
    a = req.get("a", "")
    b = req.get("b", "")
    a_ok = "да" if req.get("a_ok") == "1" else "нет"
    b_ok = "да" if req.get("b_ok") == "1" else "нет"
    await msg.reply_text(
        "Статус свадебной заявки:\n"
        f"{display_user(a)} согласие: {a_ok}\n"
        f"{display_user(b)} согласие: {b_ok}\n"
        "Подтверждение: согласен/согласна"
    )


async def marriage_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    chat_id = str(msg.chat_id)
    user = register_profile(msg.from_user)
    key, req = find_pending_marriage_for_user(chat_id, user)
    if not req:
        await msg.reply_text("Нет активной свадебной заявки.")
        return
    if user not in (req.get("a", ""), req.get("b", "")) and not is_privileged(msg.from_user):
        await msg.reply_text("Отменить может только участник заявки.")
        return
    pending_marriages.pop(key, None)
    save_json(PENDING_MARRIAGE_FILE, pending_marriages)
    await msg.reply_text("Свадебная заявка отменена.")


async def marriage_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    q = update.callback_query
    if not q or not q.data.startswith("marrycb:"):
        return
    await q.answer()
    parts = q.data.split(":", 2)
    if len(parts) != 3:
        return
    action, cbid = parts[1], parts[2]
    key = marriage_cb_map.get(cbid, "")
    if not key:
        await q.answer("Кнопка устарела.", show_alert=True)
        return
    req = pending_marriages.get(key)
    if not req:
        await q.answer("Заявка уже неактуальна.", show_alert=True)
        return
    actor = register_profile(q.from_user)
    a = req.get("a", "")
    b = req.get("b", "")
    if actor not in (a, b):
        await q.answer("Только участник пары может нажимать кнопку.", show_alert=True)
        return

    if action == "no":
        pending_marriages.pop(key, None)
        save_json(PENDING_MARRIAGE_FILE, pending_marriages)
        await q.message.reply_text(f"💔 Свадебная заявка отменена: {display_user(actor)} отказался(ась).")
        return
    ...
    if actor == a:
        ...
        req["b_ok"] = "1"
    pending_marriages[key] = req
    save_json(PENDING_MARRIAGE_FILE, pending_marriages)

    if req.get("a_ok") != "1" or req.get("b_ok") != "1":
        waiting_for = b if actor == a else a
        await q.message.reply_text(f"✅ {display_user(actor)} принял(а) брак. Ждем: {display_user(waiting_for)}")
        return

    chat_id = str(req.get("chat_id", q.message.chat_id))
    if is_user_in_marriage(chat_id, a) or is_user_in_marriage(chat_id, b):
        pending_marriages.pop(key, None)
        save_json(PENDING_MARRIAGE_FILE, pending_marriages)
        await q.message.reply_text("Свадьба отменена: один из участников уже в браке.")
        return

    pending_marriages.pop(key, None)
    save_json(PENDING_MARRIAGE_FILE, pending_marriages)
    await start_marriage_ceremony(q.message.chat_id, a, b, context)


async def start_marriage_ceremony(chat_id: int, a: str, b: str, context: ContextTypes.DEFAULT_TYPE) -> None:
    cid = uuid4().hex[:12]
    ceremony_state[cid] = {
        "chat_id": str(chat_id),
        "a": a,
        "b": b,
        "q_idx": 0,
        "answers": [{"q": q, "a": "", "b": ""} for q in CEREMONY_QUESTIONS],
        "active": True,
    }
    save_json(CEREMONY_FILE, ceremony_state)
    await context.bot.send_message(
        chat_id=chat_id,
        text=(
            "💒 Начинается свадебная церемония!\n"
            f"Пара: {display_user(a)} + {display_user(b)}\n"
            "Сейчас будет 5 вопросов. Оба отвечают да/нет."
        ),
    )
    await send_ceremony_question(cid, context)


async def send_ceremony_question(cid: str, context: ContextTypes.DEFAULT_TYPE) -> None:
    state = ceremony_state.get(cid)
    if not state or not state.get("active"):
        return
    idx = int(state.get("q_idx", 0))
    answers = state.get("answers", [])
    if idx >= len(answers):
        await finalize_marriage_ceremony(cid, context)
        return
    qtxt = answers[idx].get("q", "")
    kb = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(f"Да {display_user(state.get('a',''))}", callback_data=f"cer:yes:{cid}:a"),
                InlineKeyboardButton(f"Нет {display_user(state.get('a',''))}", callback_data=f"cer:no:{cid}:a"),
            ],
            [
                InlineKeyboardButton(f"Да {display_user(state.get('b',''))}", callback_data=f"cer:yes:{cid}:b"),
                InlineKeyboardButton(f"Нет {display_user(state.get('b',''))}", callback_data=f"cer:no:{cid}:b"),
            ],
        ]
    )
    await context.bot.send_message(
        chat_id=int(state.get("chat_id")),
        text=f"Вопрос {idx+1}/{len(answers)}:\n{qtxt}",
        reply_markup=kb,
    )


async def ceremony_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    q = update.callback_query
    if not q or not q.data.startswith("cer:"):
        return
    await q.answer()
    parts = q.data.split(":", 3)
    if len(parts) != 4:
        return
    ans, cid, who = parts[1], parts[2], parts[3]
    state = ceremony_state.get(cid)
    if not state or not state.get("active"):
        await q.answer("Церемония неактуальна.", show_alert=True)
        return
    actor = register_profile(q.from_user)
    expected = state.get("a") if who == "a" else state.get("b")
    if actor != expected:
        await q.answer("Эта кнопка не для тебя.", show_alert=True)
        return
    idx = int(state.get("q_idx", 0))
    if idx >= len(state.get("answers", [])):
        return
    row = state["answers"][idx]
    row[who] = ans
    state["answers"][idx] = row
    ceremony_state[cid] = state
    save_json(CEREMONY_FILE, ceremony_state)
    if row.get("a") and row.get("b"):
        ...
        await q.message.reply_text("Ответ принят, ждем второго участника.")


async def finalize_marriage_ceremony(cid: str, context: ContextTypes.DEFAULT_TYPE) -> None:
    state = ceremony_state.get(cid)
    if not state:
        return
    ...
    for row in state.get("answers", []):
        yes_total += 1 if row.get("a") == "yes" else 0
        yes_total += 1 if row.get("b") == "yes" else 0
    compatibility = int(yes_total * 100 / max(1, len(state.get("answers", [])) * 2))
    marriages.setdefault(chat_id, [])
    marriages[chat_id].append(
        {"type": "marriage", "members": [a, b], "date": datetime.now().isoformat(), "wedding_date": datetime.now().isoformat(), "rings_exchanged": False, "rings_date": ""}
    )
    ...
    await context.bot.send_message(
        ...
        text=(
            "💍✨ Церемония завершена!\n"
            f"{display_user(a)} и {display_user(b)} теперь в браке.\n"
            f"Совместимость пары: {compatibility}%\n\n"
            f"{_ceremony_result_table(state)}\n\n"
            "Дальше: /relation /relation_actions /ring_exchange 12k|18k|24k"
        ),
    )
...
async def braki(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    lines = [f"{' + '.join([f'@{u}' for u in m['members']])}" for m in records if m.get("type") == "marriage"]
    if not lines:
        await msg.reply_text("Нет браков")
        return
...
async def moisoyuz(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for m in records:
        ...
        if m.get("type") == "marriage":
            lines.append("Брак: " + " + ".join([f"@{u}" for u in members]))
...
async def balance(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    chat_id = str(msg.chat_id)
    mentioned = get_mentioned_or_replied(update, context)
    user = resolve_user_key_in_chat(chat_id, mentioned[0]) if mentioned else caller
    if not user:
        user = caller
    entry = daily_rewards.setdefault(user, {"coins": 0, "streak": 0, "last_claim": ""})
    await msg.reply_text(
...
def _http_get_json(url: str, timeout: int = 8) -> dict:
    req = urllib.request.Request(
        url,
        headers={"User-Agent": "Mozilla/5.0 (Mops-Farmila Bot)"},
    )
...
def _http_post_multipart(url: str, fields: dict[str, str], files: list[tuple[str, str, bytes, str]], timeout: int = 20) -> dict:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "Mozilla/5.0 (Mops-Farmila Bot)",
        },
    )
    with urllib.request.urlopen(req, timeout=timeout) as r:
        raw = r.read().decode("utf-8", errors="ignore")
...
def ocr_space_extract_text(image_bytes: bytes) -> str:
    ...
    try:
        data = _http_post_multipart(
            ...
            files=[("file", "task.jpg", image_bytes, "image/jpeg")],
            timeout=30,
        )
        parsed = data.get("ParsedResults") or []
        if not parsed:
            return ""
        txt = (parsed[0].get("ParsedText") or "").strip()
        return re.sub(r"\s{2,}", " ", txt).strip()
    except Exception:
        return ""


def web_quick_answer(query: str) -> tuple[str, str]:
    q = (query or "").strip()
    if not q:
        return "", ""
    quoted = urllib.parse.quote(q)

    # 1) Wikipedia short summary (ru preferred, then en)
    for lang in ("ru", "en"):
        try:
            wiki_url = f"https://{lang}.wikipedia.org/api/rest_v1/page/summary/{quoted}"
            data = _http_get_json(wiki_url, timeout=8)
            text = (data.get("extract") or "").strip()
            page = (data.get("content_urls", {}).get("desktop", {}).get("page") or "").strip()
            if text:
                return text, page
        except Exception:
            pass
    ...
    try:
        ddg_url = f"https://api.duckduckgo.com/?q={quoted}&format=json&no_redirect=1&no_html=1&skip_disambig=1"
        data = _http_get_json(ddg_url, timeout=8)
        answer = (data.get("AbstractText") or data.get("Answer") or "").strip()
        source = (data.get("AbstractURL") or data.get("Results", [{}])[0].get("FirstURL", "")).strip()
        if answer:
...
async def farmila_search(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    if not msg:
        return
    ...
    if not raw and msg.text:
        ...
        prefixes = ("фармила-поиск", "поиск", "гугл", "найди")
        for pref in prefixes:
            if low.startswith(pref):
                raw = msg.text[len(pref):].strip(" :-")
                break
    if not raw:
        await msg.reply_text("Формат: /farmila_search <вопрос> или текстом: фармила-поиск <вопрос>")
        return
    await msg.reply_text("🔎 Ищу ответ в интернете...")
    answer, source = web_quick_answer(raw)
    if not answer:
        await msg.reply_text("Не нашел точный ответ. Попробуй уточнить вопрос.")
        return
    if len(answer) > 1200:
        answer = answer[:1197] + "..."
    if source:
        await msg.reply_text(f"🧠 Ответ:\n{answer}\n\nИсточник: {source}")
    else:
        await msg.reply_text(f"🧠 Ответ:\n{answer}")


async def farmila_reshi(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    if not msg:
        return
    src = msg.reply_to_message or msg
    photo = src.photo[-1] if src.photo else None
    doc = src.document if src.document else None
    if not photo and not (doc and (doc.mime_type or "").startswith("image/")):
        await msg.reply_text(
            "Формат: ответь командой /farmila_reshi на фото с задачей.\n"
            "Текстом тоже можно: фармила-реши (ответом на фото)."
        )
        return
    try:
        await msg.reply_text("📷 Считываю текст с фото...")
        file_id = photo.file_id if photo else doc.file_id
        f = await context.bot.get_file(file_id)
        data = await f.download_as_bytearray()
        extracted = ocr_space_extract_text(bytes(data))
        if not extracted:
            await msg.reply_text("Не удалось распознать текст. Отправь фото четче и крупнее.")
            return
        question = extracted[:1200]
        await msg.reply_text("🧠 Думаю над решением...")
        ans, source = web_quick_answer(question)
        if not ans:
            await msg.reply_text(
                "Распознал условие, но не нашел точный ответ в сети.\n\n"
                f"Распознанный текст:\n{question[:900]}"
            )
            return
        if len(ans) > 1200:
            ans = ans[:1197] + "..."
        out = f"✅ Решение/подсказка:\n{ans}\n\n📄 Что распознано:\n{question[:600]}"
        if source:
            out += f"\n\nИсточник: {source}"
        await msg.reply_text(out)
    except Exception:
        await msg.reply_text("Ошибка обработки фото. Попробуй еще раз с более четкой картинкой.")


def _parse_whisper_query(raw: str) -> tuple[str, str]:
    text = (raw or "").strip()
    m = re.search(r"@([A-Za-z0-9_]{3,32})\s*$", text)
    if not m:
        return "", ""
    uname = m.group(1).lower()
    body = text[: m.start()].strip()
    for prefix in ("бот ", "bot ", "шепот ", "шёпот ", "whisper "):
...
async def whisper_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        "Шептун:\n"
        "1) Напиши в поле сообщения: @ВашБот текст @username\n"
        "2) Выбери карточку \"Шепот\"\n"
        "3) В чат уйдет скрытое сообщение с кнопкой\n\n"
        "Команды:\n"
        "/whisper_help — справка\n"
        "/whisper_demo @user текст — тест шепота в чат"
    )


async def whisper_demo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    if not msg:
        return
    raw = " ".join(context.args or []).strip()
    body, uname = _parse_whisper_query(raw)
    if not body or not uname:
        await msg.reply_text("Формат: /whisper_demo <текст> @username")
        return
    wid = uuid4().hex[:12]
    whisper_state[wid] = {
        "from_id": str(msg.from_user.id),
        "from_name": display_user(register_profile(msg.from_user)),
        "to_username": uname,
        "text": body,
        "created_at": str(datetime.now().timestamp()),
    }
    save_json(WHISPER_FILE, whisper_state)
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("Открыть шепот", callback_data=f"whisper:open:{wid}")]])
    await msg.reply_text(
        f"🤫 Шепот для @{uname}\n"
        f"От: {display_user(register_profile(msg.from_user))}\n"
        "Нажми кнопку, чтобы прочитать.",
        reply_markup=kb,
    )


async def whisper_send(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    if not msg:
        return
    raw = " ".join(context.args or []).strip()
    if not raw:
        await msg.reply_text("Формат: /whisper @user текст")
        return
    m = re.match(r"@([A-Za-z0-9_]{3,32})\s+(.+)$", raw, flags=re.S)
    if not m:
        await msg.reply_text("Формат: /whisper @user текст")
        return
    uname = m.group(1).lower()
    ...
    if not body:
        await msg.reply_text("Текст шепота пустой.")
        return
    wid = uuid4().hex[:12]
    whisper_state[wid] = {
        "from_id": str(msg.from_user.id),
...
async def whisper_inline_query(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    iq = update.inline_query
    if not iq:
        return
    try:
        q = (iq.query or "").strip()
        body, uname = _parse_whisper_query(q)
        if not body or not uname:
            tip = InlineQueryResultArticle(
                id="whisper_tip",
                title="Как отправить шепот",
                description="Формат: текст @username",
                input_message_content=InputTextMessageContent(
                    "Формат шепота: @ВашБот ваш текст @username"
                ),
            )
            tip2 = InlineQueryResultArticle(
                id="whisper_tip2",
                title="Если inline не работает",
                description="Используй: /whisper @user текст",
                input_message_content=InputTextMessageContent(
                    "Inline может быть отключен. Используй: /whisper @user текст"
                ),
            )
            await iq.answer([tip, tip2], cache_time=1, is_personal=True)
            return

        wid = uuid4().hex[:12]
        from_key = register_profile(iq.from_user)
        whisper_state[wid] = {
            "from_id": str(iq.from_user.id),
            "from_name": display_user(from_key),
            "to_username": uname,
            "text": body,
            "created_at": str(datetime.now().timestamp()),
        }
        ...
        result = InlineQueryResultArticle(
            id=wid,
            title=f"Шепот для @{uname}",
            description=(body[:80] + ("..." if len(body) > 80 else "")),
            input_message_content=InputTextMessageContent(
                f"🤫 Шепот для @{uname}\n"
                f"От: {display_user(from_key)}\n"
        ...
        fallback = InlineQueryResultArticle(
            id="whisper_error",
            title="Ошибка inline-шепота",
            description="Используй: /whisper @user текст",
            input_message_content=InputTextMessageContent(
                "Не удалось создать inline-шепот. Используй: /whisper @user текст"
            ),
        )
        await iq.answer([fallback], cache_time=1, is_personal=True)


async def whisper_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    q = update.callback_query
    if not q or not q.data.startswith("whisper:open:"):
        return
    wid = q.data.split(":", 2)[2]
    data = whisper_state.get(wid)
    if not data:
        await q.answer("Шепот устарел.", show_alert=True)
        return
    actor_uname = (q.from_user.username or "").lower()
    actor_id = str(q.from_user.id)
    if actor_uname != data.get("to_username") and actor_id != data.get("from_id"):
        await q.answer("Этот шепот не для тебя.", show_alert=True)
        return
    await q.answer(
        f"Шепот: {data.get('text', '')}\nОт: {data.get('from_name', 'неизвестно')}",
        show_alert=True,
    )
...
def build_quote_sticker_image(text: str, author: str) -> BytesIO | None:
    ...
    # Если текст все равно слишком длинный, обрезаем с многоточием, чтобы не превращался в "микрошрифт".
    max_lines_safe = max(6, max_body_height // max(1, line_h))
    if len(lines) > max_lines_safe:
        lines = lines[:max_lines_safe]
        lines[-1] = (lines[-1][: max(8, len(lines[-1]) - 3)] + "...").strip()
    ...
    av_x1, av_y1, av_x2, av_y2 = 30, 34, 102, 106
    draw.ellipse((av_x1, av_y1, av_x2, av_y2), fill=(79, 142, 255, 255))
    initial = (author[:1] or "Q").upper()
    init_font = _load_font(30)
    draw.text((av_x1 + 23, av_y1 + 15), initial, font=init_font, fill=(255, 255, 255, 255))

    # Author + pseudo time-like right accent.
    name_color = (255, 132, 182, 255)
    draw.text((118, 48), author[:28], font=name_font, fill=name_color)
...
def build_quote_sticker_image_with_avatar(text: str, author: str, avatar_img) -> BytesIO | None:
    ...
    try:
        ...
        av = avatar_img.convert("RGBA").resize((64, 64))
        # Circle mask for avatar.
        mask = Image.new("L", (64, 64), 0)
        md = ImageDraw.Draw(mask)
        md.ellipse((0, 0, 64, 64), fill=255)
        base.paste(av, (34, 38), mask)
        out = BytesIO()
        out.name = "quote.webp"
...
def resolve_user_key_in_chat(chat_id: str, token: str) -> str | None:
    token = norm_user(token)
    if token.startswith("id:"):
        return token
    if token.isdigit():
        return f"id:{token}"
    users = list(mops_state.setdefault("chat_users", {}).get(chat_id, []))
    users.reverse()
    for key in users:
        p = profiles.get(key, {})
        if norm_user(p.get("username")) == token:
            return key
    return resolve_user_key_from_token(token)
...
def resolve_target_user_key(update: Update, context: ContextTypes.DEFAULT_TYPE, fallback_arg_index: int = 0) -> str | None:
    ...
    mentioned = get_mentioned_or_replied(update, context)
    if mentioned:
        found = resolve_user_key_in_chat(chat_id, mentioned[0])
        if found:
            return found
    args = list(getattr(context, "args", []) or [])
    if len(args) > fallback_arg_index:
        raw = str(args[fallback_arg_index]).lstrip("@")
        return resolve_user_key_in_chat(chat_id, raw)
    return None
...
def relation_title(days: int, level: int) -> str:
    idx = min(len(RELATION_TITLES) - 1, max(0, level - 1))
    base = RELATION_TITLES[idx]
    if days >= 365:
        return f"{base} • {days//365}г вместе"
    return base
...
async def rp_action(update: Update, context: ContextTypes.DEFAULT_TYPE, verb: str, xp: int = 8) -> None:
    ...
    target_key = None
    if mentioned:
        target_key = resolve_user_key_in_chat(chat_id, mentioned[0])
...
async def relation_actions_list(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    lines = ["Действия отношений:"]
    for name, cfg in RELATION_ACTIONS.items():
        lines.append(f"- {name}: +{cfg['xp']} XP, цена {cfg['cost']} монет")
    lines.append("Использование: /relation_do <действие> @user")
    lines.append("Текстом: отн <действие> @user")
    await update.message.reply_text("\n".join(lines))


async def relation_do(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    actor = register_profile(msg.from_user)
    chat_id = str(msg.chat_id)
    raw = " ".join(context.args or []).strip()
    if not raw:
        await msg.reply_text("Формат: /relation_do <действие> @user")
        return
    mentioned = get_mentioned_or_replied(update, context)
    target_key = None
    if mentioned:
        target_key = resolve_user_key_in_chat(chat_id, mentioned[-1])
    if not target_key and msg.reply_to_message and msg.reply_to_message.from_user:
        target_key = register_profile(msg.reply_to_message.from_user)
    if not target_key:
        await msg.reply_text("Укажи партнера: @user или ответом.")
        return
    action_text = raw
    if mentioned:
        action_text = action_text.replace(f"@{mentioned[-1]}", "").strip()
    action_text = action_text.lower()
    if action_text not in RELATION_ACTIONS:
        await msg.reply_text("Неизвестное действие. Список: /relation_actions")
        return
    idx, m = find_marriage_for_user(chat_id, actor)
    if idx is None or not m or target_key not in m.get("members", []):
        await msg.reply_text("Действие доступно только внутри вашей пары.")
        return
    cfg = RELATION_ACTIONS[action_text]
    cost = int(cfg.get("cost", 0))
    if cost > 0:
        w = ensure_wallet_key(actor)
        if int(w.get("coins", 0)) < cost:
            await msg.reply_text(f"Нужно {cost} монет для этого действия.")
            return
        w["coins"] = int(w.get("coins", 0)) - cost
        daily_rewards[actor] = w
        save_json(DAILY_FILE, daily_rewards)
    rel_key = marriage_key(chat_id, m["members"][0], m["members"][1])
    rel = relations.setdefault(rel_key, {"xp": 0, "level": 1, "created": datetime.now().isoformat()})
    now_ts = int(datetime.now().timestamp())
    cooldown_min = int(RELATION_ACTION_COOLDOWN_MIN.get(action_text, 5))
    actor_cool = rel.setdefault("cooldowns", {}).setdefault(actor, {})
    last_ts = int(actor_cool.get(action_text, 0))
    left_sec = cooldown_min * 60 - max(0, now_ts - last_ts)
    if left_sec > 0:
        skip_cost = max(5, (left_sec + 59) // 60 * 2)
        await msg.reply_text(
            f"Кулдаун действия еще {left_sec//60}м {left_sec%60}с.\n"
            f"Пропуск: /relation_skip {action_text} @{profiles.get(target_key, {}).get('username','') or target_key.replace('id:','')}\n"
            f"Цена пропуска: {skip_cost} монет"
        )
        return
    rel["xp"] = int(rel.get("xp", 0)) + int(cfg.get("xp", 0))
    up = False
    while rel["xp"] >= int(rel.get("level", 1)) * 60:
        rel["xp"] -= int(rel.get("level", 1)) * 60
        rel["level"] = int(rel.get("level", 1)) + 1
        up = True
    actor_cool[action_text] = now_ts
    rel.setdefault("stats", {})["actions"] = int(rel.setdefault("stats", {}).get("actions", 0)) + 1
    relations[rel_key] = rel
    save_json(RELATION_FILE, relations)
    await msg.reply_text(
        f"{display_user(actor)} {cfg.get('verb','взаимодействует с')} {display_user(target_key)}\n"
        f"+{cfg.get('xp',0)} XP отношений\n"
        + (f"Новый уровень пары: {rel.get('level',1)} ({relation_title(1, int(rel.get('level',1)))})" if up else "")
    )


async def relation_skip(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    actor = register_profile(msg.from_user)
    chat_id = str(msg.chat_id)
    raw = " ".join(context.args or []).strip().lower()
    if not raw:
        await msg.reply_text("Формат: /relation_skip <действие> @user")
        return
    mentioned = get_mentioned_or_replied(update, context)
    target_key = None
    if mentioned:
    ...
    idx, m = find_marriage_for_user(chat_id, actor)
    if idx is None or not m or target_key not in m.get("members", []):
        await msg.reply_text("Пропуск доступен только внутри вашей пары.")
        return
    rel_key = marriage_key(chat_id, m["members"][0], m["members"][1])
    rel = relations.setdefault(rel_key, {"xp": 0, "level": 1, "created": datetime.now().isoformat()})
    now_ts = int(datetime.now().timestamp())
    ...
    left_sec = cooldown_min * 60 - max(0, now_ts - last_ts)
    if left_sec <= 0:
        await msg.reply_text("Кулдаун уже закончился, пропуск не нужен.")
        return
    skip_cost = max(5, (left_sec + 59) // 60 * 2)
    w = ensure_wallet_key(actor)
    if int(w.get("coins", 0)) < skip_cost:
        await msg.reply_text(f"Не хватает монет для пропуска. Нужно: {skip_cost}")
        return
    w["coins"] = int(w.get("coins", 0)) - skip_cost
    daily_rewards[actor] = w
    actor_cool[action_text] = 0
    relations[rel_key] = rel
    save_json(DAILY_FILE, daily_rewards)
    save_json(RELATION_FILE, relations)
    await msg.reply_text(f"Кулдаун действия «{action_text}» снят за {skip_cost} монет.")


async def relation_top(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    chat_id = str(msg.chat_id)
    rows = []
    for m in marriages.get(chat_id, []):
        if m.get("type") != "marriage":
            continue
        a, b = m.get("members", ["", ""])
        rk = marriage_key(chat_id, a, b)
        rel = relations.get(rk, {"level": 1, "xp": 0})
        rows.append((a, b, int(rel.get("level", 1)), int(rel.get("xp", 0))))
    if not rows:
        await msg.reply_text("Пока нет пар для рейтинга.")
        return
    rows.sort(key=lambda x: (x[2], x[3]), reverse=True)
    lines = ["Топ пар по отношениям:"]
    for i, (a, b, lvl, xp) in enumerate(rows[:10], start=1):
        lines.append(f"{i}. {display_user(a)} + {display_user(b)} — L{lvl} ({xp} XP)")
    await msg.reply_text("\n".join(lines))


async def relation_daily(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    user = register_profile(msg.from_user)
    chat_id = str(msg.chat_id)
    idx, m = find_marriage_for_user(chat_id, user)
    if idx is None or not m:
        await msg.reply_text("Команда доступна только для пары в браке.")
        return
    a, b = m.get("members", ["", ""])
    partner = b if user == a else a
    rk = marriage_key(chat_id, a, b)
    rel = relations.setdefault(rk, {"xp": 0, "level": 1, "created": datetime.now().isoformat()})
    today = today_str()
    daily_map = rel.setdefault("daily_claim", {})
    if daily_map.get(user) == today:
        await msg.reply_text("Ты уже получил(а) бонус пары сегодня.")
        return
    daily_map[user] = today
    both = daily_map.get(a) == today and daily_map.get(b) == today
    coins = 20 + int(rel.get("level", 1)) * 3
    xp_gain = 15 if not both else 40
    w = ensure_wallet_key(user)
    w["coins"] = int(w.get("coins", 0)) + coins
    daily_rewards[user] = w
    rel["xp"] = int(rel.get("xp", 0)) + xp_gain
    while rel["xp"] >= int(rel.get("level", 1)) * 60:
        rel["xp"] -= int(rel.get("level", 1)) * 60
        rel["level"] = int(rel.get("level", 1)) + 1
    relations[rk] = rel
    ...
    save_json(RELATION_FILE, relations)
    await msg.reply_text(
        f"Бонус пары получен: +{coins} монет, +{xp_gain} XP отношений.\n"
        f"Партнер: {display_user(partner)}\n"
        + ("Оба получили бонус сегодня: активирован усиленный XP-буст!" if both else "Пусть партнер тоже заберет бонус сегодня для буста.")
    )


async def pair_compat(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    chat_id = str(msg.chat_id)
    user = register_profile(msg.from_user)
    idx, m = find_marriage_for_user(chat_id, user)
    ...
    rk = marriage_key(chat_id, a, b)
    rel = relations.setdefault(rk, {"xp": 0, "level": 1, "created": datetime.now().isoformat()})
    lvl = int(rel.get("level", 1))
    score = min(100, 35 + lvl * 3 + random.randint(0, 20))
    await msg.reply_text(f"Индекс совместимости пары: {score}%\nТитул: {relation_title(1, lvl)}")


async def pair_story(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    chat_id = str(msg.chat_id)
    user = register_profile(msg.from_user)
    idx, m = find_marriage_for_user(chat_id, user)
    if idx is None or not m:
        await msg.reply_text("История пары доступна только в браке.")
        return
    a, b = m.get("members", ["", ""])
    scenes = [
        "устроили вечерний пикник под фонарями",
        "вместе прошли квест и поделили награду",
        "поссорились, но нашли компромисс и обнялись",
        "написали друг другу милые заметки",
        "отправились в мини-путешествие по чату",
    ]
    await msg.reply_text(f"📖 История дня: {display_user(a)} и {display_user(b)} {random.choice(scenes)}.")
...
async def trade(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not args:
        await msg.reply_text("Использование: /trade @user item_id [кол-во] или ответом: /trade item_id [кол-во]")
        return
    ...
    if args and (args[0].startswith("@") or resolve_user_key_from_token(args[0].lstrip("@"))):
        args = args[1:]
    if not args:
        await msg.reply_text("Укажи item_id и количество.")
        return
    if target == actor:
        first_mention = re.findall(r"@([A-Za-z0-9_]{3,32})", msg.text or "")
        if first_mention:
            alt = resolve_user_key_in_chat(str(msg.chat_id), first_mention[0])
            if alt and alt != actor:
                target = alt
        if target == actor:
            await msg.reply_text("Нельзя отправить обмен самому себе.")
            return
    item_id = args[0].lower()
    if item_id not in SHOP_ITEMS:
        await msg.reply_text("Неизвестный item_id. Смотри список: /shop")
        return
    qty = int(args[1]) if len(args) > 1 and str(args[1]).isdigit() else 1
    if qty < 1 or qty > 999:
        await msg.reply_text("Количество должно быть от 1 до 999.")
        return
    ...
    tcb = uuid4().hex[:10]
    trade_cb_map[tcb] = tid
    kb = InlineKeyboardMarkup(
        [[
            InlineKeyboardButton("Принять обмен", callback_data=f"tradecb:ok:{tcb}"),
            InlineKeyboardButton("Отклонить", callback_data=f"tradecb:no:{tcb}"),
        ]]
    )
    await msg.reply_text(
...
def _find_outgoing_trades(chat_id: str, user_key: str) -> list[tuple[str, dict]]:
    rows = []
    for k, t in trade_requests.items():
        if t.get("chat_id") == chat_id and t.get("from") == user_key:
            rows.append((k, t))
    return rows


async def trade_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    user = register_profile(msg.from_user)
    chat_id = str(msg.chat_id)
    incoming = [t for _, t in trade_requests.items() if t.get("chat_id") == chat_id and t.get("to") == user]
    outgoing = [t for _, t in trade_requests.items() if t.get("chat_id") == chat_id and t.get("from") == user]
    lines = [
        "Статус обменов:",
        f"Входящие: {len(incoming)}",
        f"Исходящие: {len(outgoing)}",
    ]
    if incoming:
        t = incoming[0]
        lines.append(
            f"Входящий пример: {display_user(t.get('from',''))} -> {t.get('item','?')} x{t.get('qty',1)} "
            "(принятьобмен/отклонитьобмен)"
        )
    if outgoing:
        t = outgoing[0]
        lines.append(
            f"Исходящий пример: для {display_user(t.get('to',''))} -> {t.get('item','?')} x{t.get('qty',1)} "
            "(обменотмена)"
        )
    await msg.reply_text("\n".join(lines))


async def trade_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    user = register_profile(msg.from_user)
    chat_id = str(msg.chat_id)
    rows = _find_outgoing_trades(chat_id, user)
    if not rows:
        await msg.reply_text("У тебя нет исходящих обменов.")
        return
    key, tr = rows[0]
    trade_requests.pop(key, None)
    save_json(TRADE_FILE, trade_requests)
    await msg.reply_text(
        f"Исходящий обмен отменен: {tr.get('item','?')} x{tr.get('qty',1)} для {display_user(tr.get('to',''))}"
    )


async def trade_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    q = update.callback_query
    if not q or not q.data.startswith("tradecb:"):
        return
    await q.answer()
    parts = q.data.split(":", 2)
    if len(parts) < 3:
    ...
    action = parts[1]
    cbid = parts[2]
    key = trade_cb_map.get(cbid, "")
    if not key:
        await q.answer("Кнопка обмена устарела.", show_alert=True)
        return
    tr = trade_requests.get(key)
    if not tr:
        await q.answer("Запрос обмена уже неактуален.", show_alert=True)
        return
    actor = register_profile(q.from_user)
    if action == "ok":
        if actor != tr.get("to"):
            await q.answer("Принять может только получатель.", show_alert=True)
            return
        giver = tr["from"]
        item_id = tr["item"]
        qty = int(tr.get("qty", 1))
        ...
        if int(inv_from.get(item_id, 0)) < qty:
            trade_requests.pop(key, None)
            save_json(TRADE_FILE, trade_requests)
            await q.message.reply_text("Обмен отменен: у отправителя уже нет предмета.")
            return
        inv_from[item_id] = int(inv_from.get(item_id, 0)) - qty
        if inv_from[item_id] <= 0:
            inv_from.pop(item_id, None)
        inv_to = ensure_inventory(actor)
        inv_to[item_id] = int(inv_to.get(item_id, 0)) + qty
        trade_requests.pop(key, None)
        save_json(TRADE_FILE, trade_requests)
        save_json(INVENTORY_FILE, inventories)
        await q.message.reply_text(
            f"Обмен выполнен: {display_user(giver)} передал {item_id} x{qty} для {display_user(actor)}"
        )
    elif action == "no":
        if actor not in (tr.get("to"), tr.get("from")):
            await q.answer("Отклонить может только участник обмена.", show_alert=True)
            return
        trade_requests.pop(key, None)
        save_json(TRADE_FILE, trade_requests)
        await q.message.reply_text("Обмен отклонен.")
...
async def owner_backup_data(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    if not is_owner(msg.from_user):
        return
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    dst = os.path.join(DATA_DIR, f"backup_{stamp}")
    os.makedirs(dst, exist_ok=True)
    copied = 0
    for fn in os.listdir(DATA_DIR):
        src = os.path.join(DATA_DIR, fn)
        if not os.path.isfile(src) or not fn.endswith(".json"):
            continue
        with open(src, "rb") as rf, open(os.path.join(dst, fn), "wb") as wf:
            wf.write(rf.read())
        copied += 1
    await msg.reply_text(f"Бэкап создан: {dst}\nФайлов: {copied}")


async def owner_restore_data(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = update.message
    if not is_owner(msg.from_user):
        return
    if not context.args:
        await msg.reply_text("Формат: /mxrestore backup_YYYYMMDD_HHMMSS")
        return
    folder = context.args[0].strip()
    src_dir = os.path.join(DATA_DIR, folder)
    if not os.path.isdir(src_dir):
        await msg.reply_text("Папка бэкапа не найдена.")
        return
    restored = 0
    for fn in os.listdir(src_dir):
        ...
        with open(src, "rb") as rf, open(dst, "wb") as wf:
            wf.write(rf.read())
        restored += 1
    await msg.reply_text(f"Восстановление завершено. Файлов: {restored}\nПерезапусти бота.")
...
def build_application(token: str) -> Application:
    ...
    app.add_handler(CommandHandler("trade_cancel", trade_cancel))
    app.add_handler(CommandHandler("marriage_status", marriage_status))
    app.add_handler(CommandHandler("marriage_cancel", marriage_cancel))
    ...
    app.add_handler(CommandHandler("mxbackup", owner_backup_data))
    app.add_handler(CommandHandler("mxrestore", owner_restore_data))
    ...
    app.add_handler(CommandHandler("relation_actions", relation_actions_list))
    app.add_handler(CommandHandler("relation_do", relation_do))
    app.add_handler(CommandHandler("relation_skip", relation_skip))
    app.add_handler(CommandHandler("relation_top", relation_top))
    app.add_handler(CommandHandler("relation_daily", relation_daily))
    app.add_handler(CommandHandler("pair_compat", pair_compat))
    app.add_handler(CommandHandler("pair_story", pair_story))
    ...
    app.add_handler(CommandHandler("farmila_search", farmila_search))
    app.add_handler(CommandHandler("search", farmila_search))
    app.add_handler(CommandHandler("ask", farmila_search))
    app.add_handler(CommandHandler("farmila_reshi", farmila_reshi))
    app.add_handler(CommandHandler("solve", farmila_reshi))
    app.add_handler(CommandHandler("whisper_help", whisper_help))
    app.add_handler(CommandHandler("whisper_demo", whisper_demo))
    app.add_handler(CommandHandler("whisper", whisper_send))
    app.add_handler(CommandHandler("w", whisper_send))
    ...
    # Массовые алиасы команд (без дублирования логики).
    alias_groups: dict = {
        # БАЗА
        "help": start, "menu": start, "all": start, "guide": start, "commands": start,
        "helpall": start, "bothelp": start, "starthelp": start, "manual": start,
        # Экономика
        "bal": balance, "money": balance, "coins": balance, "wallet": balance, "cash": balance,
        "eco": eco_help, "economy": eco_help, "economy_help": eco_help,
        "store": shop, "market": shop, "mall": shop,
        "inv": inventory, "bag": inventory, "items": inventory, "backpack": inventory,
        "payto": pay, "tip": pay, "sendcoins": pay,
        "dep": deposit, "wd": withdraw, "bankdep": deposit, "bankwd": withdraw,
        "top": top_players, "leaderboard": top_players, "rating": top_players,
        "quests": quest_status, "myquests": quest_status, "quest_status": quest_status,
        "ach": achievements_cmd, "achiv": achievements_cmd, "myach": achievements_cmd,
        "lotto_buy": lottery_buy, "lotto_draw": lottery_draw,
        # Браки / отношения
        "marry": brak, "marriage": brak, "wedding": brak,
        "divorce": razvod, "split": razvod,
        "marriages": braki, "myunion": moisoyuz, "myalliance": moisoyuz,
        "marry_status": marriage_status, "marry_cancel": marriage_cancel,
        "rel": relation_status, "rel_status": relation_status, "pair": relation_status,
        "rel_actions": relation_actions_list, "rel_do": relation_do, "rel_skip": relation_skip,
        "rel_top": relation_top, "rel_daily": relation_daily,
        "paircompat": pair_compat, "pairstory": pair_story, "rel_date": relation_date, "rel_gift": relation_gift,
        # Обмен / кольца
        "ringshop": rings, "myring": my_rings, "ringtrade": ring_exchange,
        "swap": trade, "exchange": trade, "giveitem": trade,
        "tradeok": accept_trade, "tradeyes": accept_trade, "tradeno": decline_trade,
        "trades": trade_status, "tradeinfo": trade_status, "tradeoff": trade_cancel,
        # Бои / рейд
        "fight": duel, "battle": duel, "challenge": duel, "pvpstart": duel,
        "duelok": accept, "duelyes": accept, "duelno": decline,
        "duelshot": shot, "pvp_help": duel_help, "pvp_top": pvptop, "pvp_stats": pvpstats,
        "raid": raid_start, "raidgo": raid_start, "raidattack": raid_hit, "raidhit": raid_hit,
        "raidinfo": raid_status, "raidstats": raid_status, "raidleader": raid_top,
        # Игры
        "guessmops": mops_guess, "mopsgame": mops_play,
        "wheel": polesudes_start, "hangman": polesudes_start,
        "sea": battleship_start, "naval": battleship_start,
        "rockpaper": rps, "rpsgame": rps,
        "quizgame": quiz, "quizstart": quiz, "quizhard": quiz2_start,
        "guessword": guess_start, "slot": slots, "casino": slots,
        "truthgame": truth_cmd, "daregame": dare_cmd, "ship": shipper_cmd, "event": day_event_cmd,
        "mafia_new": mafia_create, "mafia_add": mafia_join, "mafia_go": mafia_start,
        "mafia_round": mafia_night_start, "mafia_info": mafia_status, "mafia_end": mafia_stop,
        # Цитаты / стикеры
        "quote_sticker": quote_sticker, "quot": quote_sticker, "quotly": quote_sticker,
        "sticker": sticker_begin, "sticker_mode": sticker_begin,
        # Мопс / Хоши / Какаду
        "mhelp": mops_farmila_help, "mopsmenu": mops_farmila_help,
        "mstatus": mops_status, "mon": mops_on, "moff": mops_off,
        "hhelp": hoshi_help, "hstatus": hoshi_status, "hon": hoshi_on, "hoff": hoshi_off,
        "htip": hoshi_tip, "hquest": hoshi_quest, "hbal": hoshi_balance, "hmood": hoshi_mood,
        "duotip": duo_tip, "duoscene": duo_scene, "duohug": duo_hug,
        "kaka": kakadu_joke, "kakajoke": kakadu_joke, "kakadu": kakadu_joke, "kmood": kakadu_mood,
        "kparty": kakadu_coin_party, "kecho": kakadu_echo, "kchallenge": kakadu_challenge,
        # Отчеты / утилиты
        "reports": reports_status, "reportson": mops_reports_on, "reportsoff": mops_reports_off,
        "hreportson": hoshi_reports_on, "hreportsoff": hoshi_reports_off,
        "kreportson": kakadu_reports_on, "kreportsoff": kakadu_reports_off,
        "pong": bot_ping, "status": bot_ping, "userid": my_id,
        # Донат
        "donate_menu": donate_menu, "supportbot": donate_menu, "donatetop": donate_top,
        # Скрытые owner
        "mxcoins": owner_grant_coins, "mxxp": owner_dup_xp, "mxdup": owner_dup_coins,
        "mxpremium": owner_grant_premium, "mxreport": owner_secret_report,
        "mxsave": owner_backup_data, "mxload": owner_restore_data,
    }
    for alias_cmd, alias_handler in alias_groups.items():
        app.add_handler(CommandHandler(alias_cmd, alias_handler))
    ...
    app.add_handler(CallbackQueryHandler(trade_callback, pattern=r"^tradecb:"))
    app.add_handler(CallbackQueryHandler(marriage_callback, pattern=r"^marrycb:"))
    app.add_handler(CallbackQueryHandler(ceremony_callback, pattern=r"^cer:"))
...
def main() -> None:
    global marriages, duel_stats, war_stats, word_games, raid_states, daily_rewards, inventories, mops_state, profiles, relations, trade_requests, minigames, mod_reports, quests, achievements, lottery, bank_data, xp_data, mod_state, donate_stats, ceremony_state, pending_marriages, whisper_state
    ...
    pending_marriages = load_json(PENDING_MARRIAGE_FILE, {})
    ...
    ceremony_state = load_json(CEREMONY_FILE, {})
    whisper_state = load_json(WHISPER_FILE, {})
