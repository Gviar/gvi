...
CEREMONY_FILE = os.path.join(BASE_DIR, "marriage_ceremony.json")
...
async def relation_do(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    target_key = None
    if mentioned:
        target_key = resolve_user_key_from_token(mentioned[-1])
    if not target_key and msg.reply_to_message and msg.reply_to_message.from_user:
        target_key = register_profile(msg.reply_to_message.from_user)
    if not target_key:
...
def main() -> None:
    global marriages, duel_stats, war_stats, word_games, raid_states, daily_rewards, inventories, mops_state, profiles, relations, trade_requests, minigames, mod_reports, quests, achievements, lottery, bank_data, xp_data, mod_state, donate_stats, ceremony_state
