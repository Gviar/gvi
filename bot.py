...
def build_quote_sticker_image(text: str, author: str) -> BytesIO | None:
    ...
    bubble_x1, bubble_y1, bubble_x2 = 28, 34, 484
    max_text_width = 408
    lines = wrap_lines(body_font, max_text_width)
    while len(lines) > 9 and hasattr(body_font, "size") and body_font.size > 30:
        body_font = _load_font(body_font.size - 2)
        lines = wrap_lines(body_font, max_text_width)

    line_h = int((body_font.size if hasattr(body_font, "size") else 36) * 1.23)
    bubble_h = 92 + line_h * len(lines) + 26
    bubble_y2 = min(500, bubble_y1 + bubble_h)
    ...
    av_x1, av_y1, av_x2, av_y2 = 48, 56, 92, 100
    draw.ellipse((av_x1, av_y1, av_x2, av_y2), fill=(79, 142, 255, 255))
    initial = (author[:1] or "Q").upper()
    init_font = _load_font(24)
    draw.text((av_x1 + 13, av_y1 + 8), initial, font=init_font, fill=(255, 255, 255, 255))

    # Author + pseudo time-like right accent.
    name_color = (255, 132, 182, 255)
    draw.text((104, 58), author[:28], font=name_font, fill=name_color)
    draw.text((435, 62), "⋯", font=_load_font(26), fill=(208, 214, 255, 220))
    ...
    draw.text((56, bubble_y2 - 26), "quote", font=_load_font(18), fill=(170, 176, 202, 220))
