...
async def handle_ru_commands(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if text in game_commands:
        ...
        no_args = [
            ...
            , 'какадушутка', 'какадупати', 'какадунастроение', 'какадучеллендж', 'донат', 'донаттоп', 'обменстатус', 'обменотмена', 'бракстатус', 'бракотмена', 'отндействия', 'отнтоп', 'отндейли'
        ]
...
async def _marriage_decision_text(update: Update, text: str) -> None:
    ...
    await msg.reply_text(
        ...
        f"{display_user(a)} и {display_user(b)} теперь официально в браке!\n"
        "💍 Обменяйтесь кольцами: /ring_exchange 12k|18k|24k"
    )
...
async def marriage_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await q.message.reply_text(
        "💍✨ Брак подтвержден кнопками!\n"
        f"{display_user(a)} + {display_user(b)} теперь в браке.\n"
        "Для ритуала колец: /ring_exchange 12k|18k|24k"
    )
